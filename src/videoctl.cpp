...
//关闭流对应的解码器等
void VideoCtl::stream_component_close(VideoState *is, unsigned int stream_index)
{
...
void VideoCtl::stream_close(VideoState *is)
{
    ...
    // 清理纹理，但不清理window和renderer
        if (is->vid_texture) {
            SDL_DestroyTexture(is->vid_texture);
            is->vid_texture = NULL;  // 重要：设为NULL，避免野指针
        }
        if (is->sub_texture) {
...
{
    qDebug() <<"id: default_value" << id << ": " << default_value;
    return 0;
}
...
void VideoCtl::OnSpeed(float speed)
{
    ffp_set_property_float(FFP_PROP_FLOAT_PLAYBACK_RATE, speed);
        emit SigSpeed(speed);
}
...
void VideoCtl::update_video_pts(VideoState *is, double pts,  int serial) {
...
//                    qDebug() << "mdy audio_buf_size: " << is->audio_buf_size;
            ...
            // 应用均衡器和声道平衡
                    if (is->audio_buf && is->audio_buf_size > 0) {
                        // 获取音频参数
                        int channels = is->audio_tgt.channels;
                        int sampleRate = is->audio_tgt.freq;

                        // 根据音频格式处理
                        if (is->audio_tgt.fmt == AV_SAMPLE_FMT_FLT) {
                            // 32位浮点格式
                            float* audioData = (float*)is->audio_buf;
                            int sampleCount = is->audio_buf_size / sizeof(float);

                            // 应用均衡器
                            pVideoCtl->applyEqualizer(audioData, sampleCount, channels, sampleRate);

                            // 应用声道平衡
                            pVideoCtl->applyBalance(audioData, sampleCount, channels);

                        } else if (is->audio_tgt.fmt == AV_SAMPLE_FMT_S16) {
                            // 16位整型格式 - 需要转换为浮点处理
                            short* audioData = (short*)is->audio_buf;
                            int sampleCount = is->audio_buf_size / sizeof(short);
                            ...
                            // 应用均衡器
                            pVideoCtl->applyEqualizer(floatData.data(), sampleCount, channels, sampleRate);

                            // 应用声道平衡
                            pVideoCtl->applyBalance(floatData.data(), sampleCount, channels);

                            // 转换回16位
                            for (int i = 0; i < sampleCount; i++) {
                                float value = floatData[i];
                                value = std::max(-1.0f, std::min(1.0f, value)); // 限制范围
                                audioData[i] = (short)(value * 32767.0f);
                            }
                        }
                    }
            // 如果是纯音频模式，计算频谱
                                   if (pVideoCtl->m_bAudioOnly && is->audio_buf && is->audio_buf_size > 0) {
                                        // 根据音频格式计算频谱
                                        if (is->audio_tgt.fmt == AV_SAMPLE_FMT_FLT) {
                                            pVideoCtl->calculateSpectrum((float*)is->audio_buf,
                                                                        is->audio_buf_size / sizeof(float));
                                        } else if (is->audio_tgt.fmt == AV_SAMPLE_FMT_S16) {
                                            // 将16位整数转换为浮点数
                                            int sampleCount = is->audio_buf_size / sizeof(short);
                                            float* floatData = new float[sampleCount];
                                            for (int i = 0; i < sampleCount; i++) {
                                                floatData[i] = ((short*)is->audio_buf)[i] / 32768.0f;
                                            }
                                            pVideoCtl->calculateSpectrum(floatData, sampleCount);
                                            delete[] floatData;
                                        }
                                        pVideoCtl->m_bNeedUpdateSpectrum = true;
                                    }
        }
...
//打开流
int VideoCtl::stream_component_open(VideoState *is, unsigned int stream_index)
{
    AVFormatContext *ic = is->ic;
    ...
    default:
            // 其他类型（DATA、ATTACHMENT、UNKNOWN 等）忽略或做日志
            break;
    }
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        // 添加音频信息获取
            if (avctx) {
                emit SigAudioInfo(QString("音频编解码器: %1, 采样率: %2 Hz, 声道: %3, 比特率: %4 kb/s")
                    .arg(codec->name)
                    .arg(avctx->sample_rate)
                    .arg(avctx->channels)
                    .arg(avctx->bit_rate / 1000));
                emit SigAudioSampleRate(avctx->sample_rate);
                emit SigAudioChannels(avctx->channels);
                emit SigAudioBitrate(avctx->bit_rate / 1000);
            }
        break;
    case AVMEDIA_TYPE_VIDEO:
        m_nFrameW = 0;
...
void VideoCtl::ReadThread(VideoState *is)
{
    ...
    // 如果是音频文件，提取元数据
    if (GlobalHelper::IsMusic(is->filename)) {
        extractAudioMetadata(ic);
    }
    if (ic->pb)
...
void VideoCtl::refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
    ...
    {
        // 如果暂停状态发生变化或需要强制刷新，确保刷新一次
                if (is->paused != last_paused || is->force_refresh) {
                    is->force_refresh = 1;
...
void VideoCtl::OnPlaySeek(double dPercent)
{
    ...
    int64_t ts =0;
    if (dPercent<=1){//百分比跳转
    ts = dPercent * m_CurStream->ic->duration;
    }else{//秒跳转到目标秒数dPercent
       ts = (int64_t)(dPercent * AV_TIME_BASE);
    }
...
void VideoCtl::onGetPlayPosition()
{
    if (m_CurStream == nullptr)
    {
        GlobalVars::currentPlaytime() = 0;
        return;
    }
    ...
    if (!renderer) return;
        ...
        // 修改这里的逻辑
            if (m_bAudioOnly) {
                ...
                return; // 不进行视频渲染
                if (m_bNeedUpdateSpectrum) {
    //                drawSpectrum();
                    m_bNeedUpdateSpectrum = false;
                }
            } else if (renderer) {
                // 正常视频显示
...
//                screen_height = h;
    ...
    // 如果是纯音频模式，显示频谱
            if (m_bAudioOnly) {
                SDL_SetWindowTitle(window, "音频频谱");
                m_bNeedUpdateSpectrum = true;
            }
    return 0;
}
...
void VideoCtl::runFadeProc(int optionYype)
{   if (m_CurStream->audio_volume == 0 && optionYype==2){
        emit SigCloseWin();
        return;
    }
    if (m_fadeTimer->isActive()) return;
    m_fadeInterval = 5;
    m_runTimes = 0 ;
    m_lastVolume = m_CurStream->audio_volume;
    finishOper=optionYype;
    m_fadeTimer->start();
}
...
VideoCtl::VideoCtl(QObject *parent) :
    ...
    m_bAudioOnly(false),
    m_bNeedUpdateSpectrum(false),
    m_nSpectrumBands(32), // 默认32段
    m_pSpectrumData(nullptr),
    m_pSpectrumMutex(nullptr),
    ...
    audio_speed_convert(NULL)
{
    ...
    // 初始化频谱数据
        m_pSpectrumData = new float[m_nSpectrumBands];
        memset(m_pSpectrumData, 0, sizeof(float) * m_nSpectrumBands);
        m_pSpectrumMutex = SDL_CreateMutex();
        m_fadeTimer = new QTimer(this);
        m_fadeTimer->setInterval(m_fadeInterval); // 50ms更新一次
       connect(m_fadeTimer, &QTimer::timeout, this, &VideoCtl::onFadeTimer);
}
...
VideoCtl::~VideoCtl()
{
    ...
    if (m_pSpectrumMutex) {
            SDL_DestroyMutex(m_pSpectrumMutex);
            m_pSpectrumMutex = nullptr;
        }
...
bool VideoCtl::StartPlay(QString strFileName, WId widPlayWid)
{
    ...
    m_ready =false;
        ...
        // === 新增：安全终止正在进行的索引构建线程 ===
            if (m_indexBuildThread.joinable()) {
                m_bCancelIndexBuild = true;          // 通知线程取消
                m_indexBuildThread.join();           // 等待线程结束
                m_bCancelIndexBuild = false;         // 重置标志
            }
...
void VideoCtl::SetAudioOnlyMode(bool audioOnly)
{
    m_bAudioOnly = audioOnly;
        if (audioOnly && m_CurStream) {
            // 更新音频信息
            if (m_CurStream->audio_st && m_CurStream->audio_st->codecpar) {
                m_nAudioChannels = m_CurStream->audio_st->codecpar->channels;
                m_nSampleRate = m_CurStream->audio_st->codecpar->sample_rate;
            }
        }
}
// 计算频谱数据
void VideoCtl::calculateSpectrum(const float* audioData, int samples)
{
    if (samples < m_nSpectrumBands * 2) return;

    SDL_LockMutex(m_pSpectrumMutex);

    // 简单的FFT模拟 - 实际项目中可以使用真正的FFT库
    int samplesPerBand = samples / m_nSpectrumBands;

    for (int band = 0; band < m_nSpectrumBands; band++) {
        float sum = 0.0f;
        ...
        float rms = sqrtf(sum / samplesPerBand);

        // 转换为分贝
        float db = 20.0f * log10f(rms + 0.0001f);

        // 归一化到0-1范围
        m_pSpectrumData[band] = (db + 60.0f) / 60.0f;
        if (m_pSpectrumData[band] < 0) m_pSpectrumData[band] = 0;
        if (m_pSpectrumData[band] > 1) m_pSpectrumData[band] = 1;
    }

    SDL_UnlockMutex(m_pSpectrumMutex);

    // 发射频谱数据信号
    emit SigAudioSpectrumData(m_pSpectrumData, m_nSpectrumBands);
}

void VideoCtl::setEqualizer(const QList<int>& gains, int balance, bool enabled)
{
    m_eqEnabled = enabled;
    m_balance = balance;
    ...
    if (gains.size() == 10) {
        for (int i = 0; i < 10; i++) {
            m_eqGains[i] = gains[i];  // dB值
        }
    }

    // 标记需要更新滤波器系数
    m_eqCoefficients.clear();

    // 如果有音频流，立即更新系数
    if (m_CurStream && m_CurStream->audio_tgt.freq > 0) {
        updateEqualizerCoefficients(m_CurStream->audio_tgt.freq);
    }
}

void VideoCtl::setEqualizerEnabled(bool enabled)
{
    m_eqEnabled = enabled;
}

void VideoCtl::updateEqualizerCoefficients(int sampleRate)
{
    if (sampleRate <= 0) return;

    // 中心频率
    std::vector<float> frequencies = {60, 170, 310, 600, 1000, 3000, 6000, 12000, 14000, 16000};

    // 计算每个频段的二阶滤波器系数
    m_eqCoefficients.clear();
    m_eqCoefficients.resize(10, std::vector<float>(5, 0.0f));

    for (size_t i = 0; i < 10; i++) {
        float f0 = frequencies[i];
        float gain = m_eqGains[i];
        float Q = 1.0f;  // 带宽因子
        ...
        float b1 = (-2 * cosf(w0)) / a0;
        float b2 = (1 - alpha * A) / a0;
        float a1 = (-2 * cosf(w0)) / a0;
        float a2 = (1 - alpha / A) / a0;

        m_eqCoefficients[i][0] = b0;
        m_eqCoefficients[i][1] = b1;
        m_eqCoefficients[i][2] = b2;
        m_eqCoefficients[i][3] = a1;
        m_eqCoefficients[i][4] = a2;
    }

    // 重置滤波器实例
    if (m_CurStream && m_CurStream->audio_tgt.channels > 0) {
        int channels = m_CurStream->audio_tgt.channels;
        m_eqFilterInstances.resize(channels);
        for (int ch = 0; ch < channels; ch++) {
            m_eqFilterInstances[ch].resize(10);
            for (int band = 0; band < 10; band++) {
                auto& coeffs = m_eqCoefficients[band];
                auto& filter = m_eqFilterInstances[ch][band];
                filter.b0 = coeffs[0];
                filter.b1 = coeffs[1];
                filter.b2 = coeffs[2];
                filter.a1 = coeffs[3];
...
void VideoCtl::applyBalance(float* audioData, int samples, int channels)
{
    if (channels != 2 || m_balance == 0) return;
    ...
    if (m_balance < 0) {
        // 偏左
        rightGain = 1.0f - (-m_balance / 100.0f);
    } else {
        // 偏右
        leftGain = 1.0f - (m_balance / 100.0f);
    }

    for (int i = 0; i < samples; i += channels) {
        audioData[i] *= leftGain;     // 左声道
        audioData[i + 1] *= rightGain; // 右声道
    }
}

// 修改 applyEqualizer 函数：

void VideoCtl::applyEqualizer(float* audioData, int samples, int channels, int sampleRate)
{
    if (!m_eqEnabled || channels <= 0) return;

    // 检查是否需要更新滤波器系数
    if (m_eqCoefficients.empty() && sampleRate > 0) {
        updateEqualizerCoefficients(sampleRate);
    }

    if (m_eqFilterInstances.size() != (unsigned int)channels) {
        m_eqFilterInstances.resize(channels);
        for (int ch = 0; ch < channels; ch++) {
            m_eqFilterInstances[ch].resize(10);
            for (unsigned int band = 0; band < 10; band++) {
                if (band < m_eqCoefficients.size()) {
                    auto& coeffs = m_eqCoefficients[band];
                    auto& filter = m_eqFilterInstances[ch][band];
                    filter.b0 = coeffs[0];
                    filter.b1 = coeffs[1];
                ...
                m_eqFilterInstances[ch][band].reset();
            }
        }
    }
    ...
    for (int ch = 0; ch < channels; ch++) {
        for (int i = ch; i < samples; i += channels) {
            ...
            // 依次通过所有频段的滤波器
            for (unsigned int band = 0; band < 10; band++) {
                if ((unsigned int)ch < m_eqFilterInstances.size() && band < m_eqFilterInstances[ch].size()) {
                    sample = m_eqFilterInstances[ch][band].process(sample);
                }
            }
...
void VideoCtl::SetVideoDisplaySize(int width, int height)
{
    screen_width = width;
    screen_height = height;

    if (m_CurStream) {
        m_CurStream->width = width;
        m_CurStream->height = height;
...
void VideoCtl::extractAudioMetadata(AVFormatContext* formatCtx)
{
    if (!formatCtx) return;

    AVDictionary* metadata = formatCtx->metadata;
    QString title, artist, album, genre;
    int year = 0, bitrate = 0, sampleRate = 0;
    ...
    while ((tag = av_dict_get(metadata, "", tag, AV_DICT_IGNORE_SUFFIX))) {
        ...
        if (key.contains("title")) {
            ...
            // 尝试解析年份
            QString yearStr = value;
            if (yearStr.length() >= 4) {
                // 尝试提取前4位数字
                QRegularExpression re("\\b(\\d{4})\\b");
                QRegularExpressionMatch match = re.match(yearStr);
                if (match.hasMatch()) {
    ...
    // 使用 formatCtx 的比特率（平均比特率）
    if (formatCtx->bit_rate > 0) {
        bitrate = formatCtx->bit_rate / 1000; // 转换为kbps
    } else {
        // 如果没有全局比特率，尝试从音频流获取
        for (unsigned int i = 0; i < formatCtx->nb_streams; i++) {
            ...
            if (stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
                if (stream->codecpar->bit_rate > 0) {
                    bitrate = stream->codecpar->bit_rate / 1000;
                }
                break;
            }
        }
    ...
    for (unsigned int i = 0; i < formatCtx->nb_streams; i++) {
        ...
        if (stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            qDebug() << "找到音频流" << i << "，采样率:" << sampleRate << "Hz";
            break;
        }
    }
...
//    qDebug() << "  文件比特率:" << formatCtx->bit_rate << "bps";
    ...
    // 提取专辑封面
    QPixmap cover = extractAlbumArt(formatCtx);
    //if (!cover.isNull()) {
        emit SigAudioCoverReceived(cover);
    //}
}

// 提取专辑封面
QPixmap VideoCtl::extractAlbumArt(AVFormatContext* formatCtx)
{
    if (!formatCtx) return QPixmap();
    ...
    for (unsigned int i = 0; i < formatCtx->nb_streams; i++) {
        ...
        if (stream->disposition & AV_DISPOSITION_ATTACHED_PIC) {
            ...
            // 使用 QImage 加载图片数据
            QImage image;
            if (image.loadFromData((const uchar*)pkt.data, pkt.size)) {
                ...
                if (codec) {
                    ...
                    if (codecCtx) {
                        ...
                        if (avcodec_open2(codecCtx, codec, NULL) >= 0) {
                            AVFrame* frame = av_frame_alloc();
                            AVPacket packet;
                            av_packet_ref(&packet, &stream->attached_pic);
                            ...
                            if (ret >= 0) {
                                ...
                                if (ret >= 0) {
                                    ...
                                    if (swsCtx) {
                                        ...
                                        // 分配内存
                                        int size = av_image_alloc(data, linesize,
                                            frame->width, frame->height, AV_PIX_FMT_RGB32, 1);

                                        if (size > 0) {
                                            sws_scale(swsCtx, frame->data, frame->linesize,
                                                      0, frame->height, data, linesize);

                                            QImage qimage(data[0], frame->width, frame->height,
                                                         linesize[0], QImage::Format_RGB32);
    ...
    // 如果没有找到附件图片流，尝试从metadata中查找
    AVDictionaryEntry* tag = av_dict_get(formatCtx->metadata, "metadata_block_picture", nullptr, 0);
    if (!tag) {
        tag = av_dict_get(formatCtx->metadata, "APIC", nullptr, 0);
    }

    if (tag) {
        // 尝试解码base64
        QString tagValue = QString(tag->value);
        if (tagValue.contains("base64")) {
            // 提取base64数据
            QString base64Data = tagValue.split(',').last();
            ...
            // 直接尝试加载
            QImage image;
            if (image.loadFromData((const uchar*)tag->value, strlen(tag->value))) {
                return QPixmap::fromImage(image);
            }
        }
    }
    ...
    while ((entry = av_dict_get(formatCtx->metadata, "", entry, AV_DICT_IGNORE_SUFFIX))) {
        ...
        if (key.contains("picture") || key.contains("cover")) {
            qDebug() << "找到封面元数据标签：" << key;
            // 这里可以进一步处理封面数据
        }
    }
...
void VideoCtl::onFadeTimer()
{
    static int currentVolume = m_lastVolume;
    // 计算音量（淡出时快速衰减，淡入时平滑恢复）
    m_fadeInterval += 3 ;
    currentVolume =currentVolume -m_lastVolume / (10.0+m_runTimes);
           m_runTimes++;
    //qDebug() << "m_runTimes:" << m_runTimes << "准备淡出音量，目前音量是：" << m_lastVolume << "减到音量是：" << currentVolume;
           if (currentVolume > 0){
               // 设置音量
               m_CurStream->audio_volume = currentVolume;
               m_fadeTimer->setInterval(m_fadeInterval>100? 100: m_fadeInterval );
           } else {
               m_CurStream->audio_volume = 0;
               m_fadeInterval = 5;
               m_runTimes = 0 ;
              ...
              if (finishOper == 0){//暂停
                  toggle_pause(m_CurStream);
                  emit SigPauseStat(m_CurStream->paused);
              }else if (finishOper == 2){//退出
                 emit SigCloseWin();
              }
               m_fadeTimer->stop();
               QTimer::singleShot(200, this, [this]() {
                  m_CurStream->audio_volume = m_lastVolume;
                qDebug() << "淡出音量完成，恢复音量是：" << m_CurStream->audio_volume;
               });
           }
}

int VideoCtl::getFileType(const QString& strFileName)
{
    // 首先检查文件是否存在且可读
    ...
    int ret = avformat_open_input(&formatCtx, strFileName.toUtf8().constData(), nullptr, nullptr);
    if (ret < 0) {
        char errorMsg[256];
    ...
    avformat_close_input(&formatCtx);
    // 如果既没有音频流也没有视频流，则无法播放
    if (hasVideoStream) {
        return 2;
    }else if (hasAudioStream)return 1;
    return -3;
}
// 设置逐帧播放模式
void VideoCtl::SetFrameStepMode(bool enabled)
{
    if (enabled) {
        if (m_bFrameStepMode)return;
        // 进入逐帧播放模式
        if (m_CurStream) {
            // 保存当前播放状态
            m_savedPosition = get_master_clock(m_CurStream);
            if (!m_bFrameStepMode)
            m_savedVolume = m_CurStream->audio_volume * 1.0 / SDL_MIX_MAXVOLUME;

            // 暂停视频
            if (!m_CurStream->paused) {
                toggle_pause(m_CurStream);
                emit SigPauseStat(true);
        ...
        if (m_CurStream) {
            // 恢复音频
            m_CurStream->audio_volume = m_savedVolume * SDL_MIX_MAXVOLUME;
            qDebug() << "恢复的音量值 是：－－－－－－－－－－－－－－－－－－－－" << m_savedVolume;
            // 清除步进标志
            m_CurStream->step = 0;
    ...
    m_bFrameStepMode = enabled;
}

// 向前步进一帧
void VideoCtl::StepFrameForward()
{
    if (!m_CurStream || !m_bFrameStepMode) return;
...
void VideoCtl::StepFrameBackward()
{
    if (!m_CurStream || !m_bFrameStepMode) return;
    qDebug() << "=== 向后步进一帧（逆向播放方法）===";
    // 确保在暂停状态
    if (!m_CurStream->paused) {
        toggle_pause(m_CurStream);
        emit SigPauseStat(true);
    ...
    // 获取当前时间
    double currentTime = get_master_clock(m_CurStream);
    if (std::isnan(currentTime)) {
        // 如果时钟无效，尝试从帧队列获取
        SDL_LockMutex(m_CurStream->pictq.mutex);
        if (m_CurStream->pictq.rindex_shown && m_CurStream->pictq.size > 0) {
            Frame* frame = &m_CurStream->pictq.queue[m_CurStream->pictq.rindex];
            currentTime = frame->pts;
        }
        SDL_UnlockMutex(m_CurStream->pictq.mutex);
    }
    if (std::isnan(currentTime) || currentTime <= 0) {
        currentTime = 0;
    }
    qDebug() << "当前时间:" << currentTime;
    // 获取帧率
    double frameDuration = 0.04; // 默认25fps
    if (m_CurStream->video_st) {
        AVRational frameRate = av_guess_frame_rate(
            m_CurStream->ic, m_CurStream->video_st, NULL);
        if (frameRate.num && frameRate.den) {
            frameDuration = 1.0 / av_q2d(frameRate);
        }
    }
    // 计算向前跳转的时间（比如0.5秒，确保能跳转到前一个关键帧）
    double jumpBackTime = frameDuration; // 跳转0.5秒
    double targetTime = currentTime - jumpBackTime;
    if (targetTime < 0) targetTime = 0;
    qDebug() << "先向前跳转" << jumpBackTime << "秒到:" << targetTime;
    // 执行跳转
    int64_t seekPos = (int64_t)(targetTime * AV_TIME_BASE);
    m_FrameSeek=true;
    stream_seek(m_CurStream, seekPos, 0);
    // 等待跳转完成
    SDL_Delay(100);
        m_FrameSeek =false;
    // 步进到目标帧
...
void VideoCtl::buildKeyframeIndexForFile(const QString& fileName)
{
    ...
    double max_interval = 0.0,sum = 0.0;
    double interval=0.0;
    ...
    while (av_read_frame(fmt_ctx, &pkt) >= 0) {
        ...
        if (pkt.stream_index == video_stream && (pkt.flags & AV_PKT_FLAG_KEY)) {
           ...
           if(analysis.totalKeyFrames==0){
                ...
                m_bKeyFrameSparse = keyframe_count;
                GlobalHelper::isKeyFrameSparse() = analysis.isKeyFrameSparse;
                m_KeyFrameInfo=analysis;
                qDebug() << "扫描包数:" << i
...
double VideoCtl::getNextKeyframeAfter(double currentPts)
{
    ...
    if(*it - currentPts< 1 && it != m_keyframePTS.end()-1)it++;
